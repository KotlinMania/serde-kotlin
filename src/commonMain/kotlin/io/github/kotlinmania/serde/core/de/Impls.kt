...
private fun invalidSigned(
    v: Long,
    exp: Expected,
): SerdeResult<Nothing> = SerdeResult.failure(Error.invalidValue(Unexpected.Signed(v), exp))

private fun invalidUnsigned(
    v: ULong,
    exp: Expected,
): SerdeResult<Nothing> = SerdeResult.failure(Error.invalidValue(Unexpected.Unsigned(v), exp))
...
private fun invalidOther(
    label: String,
    exp: Expected,
): SerdeResult<Nothing> = SerdeResult.failure(Error.invalidValue(Unexpected.Other(label), exp))
...
private data object CharVisitor : Visitor<Char> {
    ...
    override fun visitStr(v: String): SerdeResult<Char> =
        if (v.length == 1) SerdeResult.success(v[0]) else SerdeResult.failure(Error.invalidValue(Unexpected.Str(v), this))
}
                        ...
                        serdeCatching {
                            val first =
                                access.nextElementSeed(SeedFromDeserialize(firstDeserialize)).getOrThrow()
                                    ?: throw SerdeException(Error.invalidLength(0, this))
                            val second =
                                access.nextElementSeed(SeedFromDeserialize(secondDeserialize)).getOrThrow()
                                    ?: throw SerdeException(Error.invalidLength(1, this))
                            first to second
                        }
                },
            )
                        ...
                        serdeCatching {
                            ...
                            val third =
                                access.nextElementSeed(SeedFromDeserialize(thirdDeserialize)).getOrThrow()
                                    ?: throw SerdeException(Error.invalidLength(2, this))
                ...
                override fun <A> visitMap(access: A): SerdeResult<Duration>
                    ...
                    serdeCatching {
                        ...
                        while (true) {
                            ...
                            when (key) {
                                "secs" -> {
                                    if (secs != null) throw SerdeException(Error.duplicateField("secs"))
                                    secs = access.nextValueSeed(SeedFromDeserialize(U64Deserialize)).getOrThrow()
                                }
                                "nanos" -> {
                                    if (nanos != null) throw SerdeException(Error.duplicateField("nanos"))
                                    nanos = access.nextValueSeed(SeedFromDeserialize(U32Deserialize)).getOrThrow()
                                }
                                else -> throw SerdeException(Error.unknownField(key, listOf("secs", "nanos")))
                            }
                        }
                        val s = secs ?: throw SerdeException(Error.missingField("secs"))
                        val n = nanos ?: throw SerdeException(Error.missingField("nanos"))
                ...
                override fun <A> visitMap(access: A): SerdeResult<Instant>
                    ...
                    serdeCatching {
                        ...
                        while (true) {
                            ...
                            when (key) {
                                "secs_since_epoch" -> {
                                    if (secs != null) throw SerdeException(Error.duplicateField("secs_since_epoch"))
                                    secs = access.nextValueSeed(SeedFromDeserialize(U64Deserialize)).getOrThrow()
                                }
                                "nanos_since_epoch" -> {
                                    if (nanos != null) throw SerdeException(Error.duplicateField("nanos_since_epoch"))
                                    nanos = access.nextValueSeed(SeedFromDeserialize(U32Deserialize)).getOrThrow()
                                }
                        ...
                        val s = secs ?: throw SerdeException(Error.missingField("secs_since_epoch"))
                        val n = nanos ?: throw SerdeException(Error.missingField("nanos_since_epoch"))
            ...
            deserializer.deserializeIdentifier(
                object : Visitor<String> {
                    ...
                    override fun visitStr(v: String): SerdeResult<String> =
                        if (v in fields) SerdeResult.success(v) else SerdeResult.failure(Error.unknownField(v, fields))
                    ...
                    override fun visitBytes(v: ByteArray): SerdeResult<String> {
                        ...
                        return if (s in fields) {
                            ...
                            SerdeResult.failure(Error.unknownField(fromUtf8LossyNoAlloc(v), fields))
                        }
                    }
                },
            )
