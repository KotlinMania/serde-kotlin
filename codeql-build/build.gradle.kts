...
fun propertyValue(
    name: String,
    default: String,
): String = parentProperties.getProperty(name) ?: providers.gradleProperty(name).orNull ?: default
...
fun registerCodeqlCompileTask(
    ...
    tasks.register<JavaExec>(taskName) {
        ...
        val sourceDirs =
            listOf(
                sourceRoot.dir("src"),
                sourceRoot.dir("serde_core/src"),
                sourceRoot.dir("serde_derive/src"),
            )
        val commonSources =
            files(
                sourceDirs.flatMap { dir ->
                    codeqlKotlinCommonSourceSetNames.map { sourceSetName ->
                        fileTree(dir.dir("$sourceSetName/kotlin")) { include("**/*.kt") }
                    }
                },
            )
        val sources =
